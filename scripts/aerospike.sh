...
case "$DEFAULT_KEY" in
	...
	stat_read_latency_gt100 | stat_read_latency_gt250 | stat_read_latency_gt50 | stat_write_latency_gt100 | stat_write_latency_gt250 | stat_write_latency_gt50 )
		...
		;;
	device_free_pct | memory_free_pct | evicted_objects | objects | expired_objects | memory_used_bytes | client_write_success | client_write_error | client_read_success | client_read_error )
		res=`/usr/bin/asinfo -h $DEFAULT_HOST -p $DEFAULT_PORT -v namespace/$OPT2 | tr ';' '\n' | tr -d ' ' | grep "^$DEFAULT_KEY=" | awk -F '=' '{print $2;}'`
		if [[ "$res" == "" ]] ; then
			res='0'
		fi
		echo $res
		;;
	namespaces )
		res=`/usr/bin/asinfo -h $DEFAULT_HOST -p $DEFAULT_PORT -v namespaces`
		for x in $sets ; do
			if [ "$res" != "" ] ; then
				res=$res','
			fi
